 ...
 * @var admin\models\BreedSearch $searchModel
 * @var admin\models\GoodType $model
 */
$breedModel = new GoodBreed();
$breedModel->type = $model->id;
...
GridView::widget([
    ...
    'filterUrl' => Url::toRoute(['view', 'id' => $model->id, 'key' => 'breed']),
...
//                'refreshGrid'=>true,
            ...
            'editableOptions'=>[
                'asPopover' => true,
                'formOptions'=>[
                    'action' => Url::toRoute(['type/update-child', 'key' => 'breed']),
                ],
                'size'=>'sm',
                'options' => ['class'=>'form-control', 'placeholder'=>'输入品种名称']
            ],
            'filterType'=>GridView::FILTER_SELECT2,
            'filterWidgetOptions'=>[
                'data'=>GoodBreed::GetAllBreeds($model->id),
            ...
            'buttons' => [
                ...
                'delete' => function ($url, $model) {
                    if($model->is_active == 0){
                        return Html::a(Yii::t('app', 'Up'), ['child-delete', 'key' => 'breed', 'id' => $model->id], [
                            'title' => Yii::t('app', '上架该检索'),
                            'class' => 'btn btn-success btn-xs',
                            'data-confirm' => '确认上架该检索？',
                        ]);
                    }else{
                        return Html::a(Yii::t('app', 'Down'), ['child-delete', 'key' => 'breed', 'id' => $model->id], [
...
\yii\bootstrap\Modal::begin([
    ...
    'header' => '<h4 class="modal-title">新增品种</h4>',
]);
echo $this->render('_breedcreate',['model'=>$breedModel]);
\yii\bootstrap\Modal::end();
?>
<script>
    $("#breed-modal").on("hidden.bs.modal", function(){
        $("#breed-form")[0].reset();//重置表单
    });
</script>