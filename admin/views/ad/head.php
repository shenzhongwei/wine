...
<div class="ad-list-index">
    ...
    echo GridView::widget([
                ...
                'buttons' => [
                    ...
                    'update' => function ($url, $model) {
                        return Html::a(Yii::t('app','Update'), $url, [
                            ...
                            'data-target'=>'#head-modal',
                            'title' => Yii::t('app', '编辑'),
                            'class' => 'btn btn-primary btn-xs update',
                        ]);
                    },
                    'delete' => function ($url, $model) {
                        return Html::a(Yii::t('app','Delete'), ['delete','name'=>'head','id'=>$model->id], [
        ...
        'toolbar'=> [
            ['content'=>
                $count>=5 ? '':Html::a('<i class="glyphicon glyphicon-plus">添加首页头部广告</i>', ['#'],[
                    'id'=>'head-add',
                    'data-pjax'=>0,
                    'type'=>'button',
                    'title'=>'添加头部广告',
                    ...
                    'data-toggle' => 'modal',    //弹框
                    'data-target' => '#head-modal',    //指定弹框的id
                ])
            ],
        ],
        'responsive'=>false,
...
<script language="JavaScript">
    ...
    function init() {
        ...
        $('.add').on('click', function () {  //查看详情的触发事件
            ...
            $.get(toRoute('ad/create'), { key:'head'  },
                function (data) {
                    $('#head-modal').find('.modal-body').html(data);  //给该弹框下的body赋值
                }
            );
        });
        $('.update').on('click', function () {  //查看详情的触发事件
            $('.ad-list-form').remove();
            $('#head-modal').find('.modal-title').html('编辑首页头部广告');
